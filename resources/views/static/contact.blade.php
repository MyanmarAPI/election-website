@extends('layouts.default')

@section('title', 'Contact')

@section('content')
	
	<div class="row app-container mg-top">
		<div class="col s12 box-content white" id="about">
			<h4>Contact Us</h4>

			<p>If you interesting to support this movement please reach us through our Facebook Group at <a href="https://www.facebook.com/MaePaySoh">https://www.facebook.com/MaePaySoh</a></p>

			<h4 id="submit-your-app">Submit Your Application</h4>
			<p>You can also submit your application information created using Myanmar Election API. We will publicize and promote each application that uses Myanmar Election API . </p>
			<p>The trick is as follows :</p>
			<ul class="bullet-list">
				<li>Applications must be made using data from the API Election</li>
				<li>
					Include the following items
					<ul class="bullet-list-disc">
						<li>Application Icon</li>
						<li>Application Screenshoot</li>
						<li>Description of the application</li>
						<li>URL where the application published applications ( example : Play Store URL )</li>
					</ul>
				</li>
			</ul>
		</div>
	</div>
@endsection